...
{	
	...
	public class MMCarousel : MonoBehaviour
	{
		...
		[Header("Focus")]
		/// If this is true, the mouse will be forced on when 
		public bool ShowMouseOnStart = true;
		...
		protected virtual void HandleFocus()
		{
			if (!_lerping && Time.timeSinceLevelLoad > 0.5f)
			{
				if (EventSystem.current.currentSelectedGameObject != null)
				{
					...
					if (UICamera.WorldToScreenPoint(EventSystem.current.currentSelectedGameObject.transform.position).x + _elementWidth/2 > Screen.width)
					{
						MoveRight ();
					}	
				}
